...
export const DEFAULT_CATEGORIES: CategoriaPersonalizada[] = [
  // Gastos
  { id: 'cat-g-1', nombre: 'Comida', icono: 'Utensils', color: '#f97316', type: 'expense', is_default: true },
  { id: 'cat-g-2', nombre: 'Transporte', icono: 'Car', color: '#3b82f6', type: 'expense', is_default: true },
  { id: 'cat-g-3', nombre: 'Salud', icono: 'HeartPulse', color: '#f43f5e', type: 'expense', is_default: true },
  { id: 'cat-g-4', nombre: 'Servicios', icono: 'Zap', color: '#eab308', type: 'expense', is_default: true },
  { id: 'cat-g-5', nombre: 'Arriendo', icono: 'Home', color: '#10b981', type: 'expense', is_default: true },
  { id: 'cat-g-6', nombre: 'Entretenimiento', icono: 'Gamepad2', color: '#a855f7', type: 'expense', is_default: true },
  // Ingresos
  { id: 'cat-i-1', nombre: 'Nómina', icono: 'Briefcase', color: '#10b981', type: 'income', is_default: true },
  { id: 'cat-i-2', nombre: 'Freelance', icono: 'Laptop', color: '#6366f1', type: 'income', is_default: true },
  { id: 'cat-i-3', nombre: 'Ventas', icono: 'ShoppingCart', color: '#06b6d4', type: 'income', is_default: true },
];
...
export function getCategoryStyle(categoriaName: string, customCategories: CategoriaPersonalizada[] = []) {
  const allCategories = customCategories.length > 0 ? customCategories : DEFAULT_CATEGORIES;
  const cat = allCategories.find(c => c.nombre === categoriaName) || DEFAULT_CATEGORIES.find(c => c.nombre === 'Comida') || allCategories[0];
  
  const matchedColor = AVAILABLE_COLORS.find(c => c.hex === cat?.color) || AVAILABLE_COLORS[AVAILABLE_COLORS.length - 1]; // Slate as fallback
...
export function enrichMovimiento(mov: any, categorias: any[], bolsillos: any[]) {
  ...
  let secondaryInfo = safeCategoria.charAt(0).toUpperCase() + safeCategoria.slice(1);
  let displayName = 'Movimiento';
  ...
  if (mov.tipo === 'ahorro' || safeCategoria === 'ahorro') {
    ...
    displayName = mov.descripcion || 'Ahorro';
    secondaryInfo = 'Ahorros';
  } else if (isBolsillo || isRetiroBolsillo) {
    const prefix = isRetiroBolsillo ? 'retiro_bolsillo_' : 'bolsillo_';
    const bolsilloId = safeCategoria.replace(prefix, '');
    const bolsillo = bolsillos.find((b: any) => String(b.id) === bolsilloId);
    if (bolsillo) {
      const colorObj = AVAILABLE_COLORS.find(c => c.hex === bolsillo.color) || AVAILABLE_COLORS[0];
      catStyle = {
        bgClass: colorObj.bg,
      ...
      secondaryInfo = isRetiroBolsillo ? 'Salida de ahorro' : 'Depósito a ahorro';
    } else {
      displayName = isRetiroBolsillo ? 'Salida de ahorro' : 'Ahorro';
      secondaryInfo = isRetiroBolsillo ? 'Retiro' : 'Ahorros';
    }
  } else {
    if (mov.tipo === 'ingreso') {
    ...
    displayName = safeDescripcion ? safeDescripcion.charAt(0).toUpperCase() + safeDescripcion.slice(1) : 'Movimiento';
  }
